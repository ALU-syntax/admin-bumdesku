...
class Driver extends REST_Controller
{
    ...
    public function cekberkas_post()
    {
        ...
        $id = $data->id;
        $response = $this->Driver_model->cekberkas($id);
        if($response)
        {
            $message = array("status"=>$response, "resultcode"=>"00");
        }
        else
        {
            $message = array("status"=>"", "resultcode"=>"01");
        }
    ...
    function wilayahkerja_post()
    {
        ...
        $id = $data->iduser;
        $response = $this->Driver_model->wilayahkerja($id);
        if($response)
        {
            $message = array(
              "resultcode"=>"00",
              "nama_cabang" => $response
            );
        }
        else 
        {
    ...
    function updatewilayah_post()
    {
        ...
        $id = $data->iduser;
        $wilayah = $data->idcabang; 
        $response = $this->Driver_model->updatewilayah($id, $wilayah);
        if($response)
        {
            $message = array(
              "pesan" => 'Wilayah Kerja Berhasil Diupdate',
              "resultcode"=> "00"
            );    
        }
        else
    ...
    function cekwilayah_post() 
    {
        ...
        $id = $data->id;
        $response = $this->Driver_model->cekwilayah($id);
        $message = array(
          "status" => $response,
        );
    ...
    function accept_post()
    {
        ...
        if ($cek_login->num_rows() > 0) {
            ...
            if ($acc_req['status']) {
                ...
                $tokenpelanggan = $this->Driver_model->idpelanggan($dec_data->id_transaksi);
                
                $title= 'Order Diterima Driver';
                $pesan = ' Ok OTW sesuai Titik';
                $this->load->model('Notification_model');
                $this->Notification_model->sendnotiftodriver($tokenpelanggan, $title, $pesan);
        ...
        if ($check_exist) {
            ...
            if ($dec_data->checked == "true") {
                ...
                $data_signup = array(
                    ...
                    'referal_source' => $dec_data->referal
                );
                ...
                $imagekendaraan = $dec_data->fotokendaraan;
                $namafotokendaraan = time() . '-' . rand(0, 99999) . ".jpg";
                $pathkendaraan = "images/fotoberkas/kendaraan/" . $namafotokendaraan;
                file_put_contents($pathkendaraan, base64_decode($imagekendaraan));
                ...
                if ($signup) {
                    $this->Pelanggan_model->referalprocess($dec_data->referal);
                    $message = array(
                        'code' => '200',
                        'message' => 'success',
    ...
    public function updateregion_post()
    {
        ...
        $imagekendaraan = $data->kendaraan;
        $namafotokendaraan = time() . '-' . rand(0, 99999) . ".jpg";
        $pathkendaraan = "images/fotoberkas/kendaraan/" . $namafotokendaraan;
        file_put_contents($pathkendaraan, base64_decode($imagekendaraan));
        
        $databerkas = array("foto_stnk" => $namafotostnk, "foto_kendaraan"=> $namafotokendaraan);
        $response = $this->Driver_model->updateregion($databerkas, $id_driver);
        if($response)
        {
            $message = array("resultcode" => "00", "pesan"=>"sukses");
        }
        else
        {
            $message = array("resultcode" => "01", "pesan"=>"failed");
        }
    ...
    public function getpoin_post() {
        ...
        $poin = $this->Pelanggan_model->Hitungpoin($id);
        if($poin) 
        {
            $message = array(
                'code' => '200',
    ...
    public function driverlocation_post()
    {
        ...
        $response = $this->Driver_model->driverlocation($id_driver);
        $message = array(
    ...
    public function listnotif_post()
    {
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.0 401 Unauthorized");
        ...
        $this->load->model('Notification_model');
        $response = $this->Notification_model->getnotiflist($type);
