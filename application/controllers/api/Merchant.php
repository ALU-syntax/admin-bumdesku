...
class Merchant extends REST_Controller
{
    ...
    public function getwilayah_post()
    {
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.0 401 Unauthorized");
    ...
    function kategorimerchant_post()
    {
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.0 401 Unauthorized");
        ...
        $fitur = $this->Merchantapi_model->fitur_merchant_active($idwilayah);
    ...
    function kategorimerchantbyfitur_post()
    {
        ...
        $data = file_get_contents("php://input");
        $decoded_data = json_decode($data);
        $idwilayah = $decoded_data->wilayah;
