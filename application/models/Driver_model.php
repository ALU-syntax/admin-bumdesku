...
class Driver_model extends CI_model
{
    ...
    public function cekberkas($id)
    {
        ...
        $query = $this->db->get('berkas_driver')->row();
        
        $stnk = $query->foto_stnk;
        $kendaraan = $query->foto_kendaraan;
        
        if(empty($stnk) || empty($kendaraan))
        {
            return "kosong";
        }
        else
    ...
    public function updateregion($databerkas, $id_driver)
    {
        $this->db->where('id_driver', $id_driver);
        $query =  $this->db->update('berkas_driver', $databerkas);
        return $query;
        
    }
    
    
    public function wilayahkerja($id)
    {
        $this->db->where('id', $id);
        $query = $this->db->get('driver')->row();
        $id_wilayah = $query->wilayah;
        
        $this->db->where('id', $id_wilayah);
        $query2 = $this->db->get('list_cabang')->row();
        return $query2->nama_cabang;
    ...
    public function updatewilayah($id, $wilayah)
    {
        $this->db->where('id', $id);
        $query = $this->db->update('driver', array('wilayah'=> $wilayah));
        return $query;
    }
    ...
    public function signup($data_signup, $data_kendaraan, $data_berkas)
    {
        ...
        $datasignup = array(
            ...
            'kendaraan' => $inserid,
            'status' => '0',
            'referal_code' => $data_signup['referal_code'],
            'referal_source' => $data_signup['referal_source']
        );
        ...
        $databerkas = array(
            ...
            'foto_kendaraan' => $data_berkas['foto_kendaraan'],
    ...
    public function accept_request($cond)
    {
        ...
        if ($cek_once->num_rows() > 0) {
            ...
            if ($this->db->affected_rows() > 0) {
                $this->db->where('id', $cond['id_driver']);
                $qdriver = $this->db->get('driver')->row();
                $wil = $qdriver->wilayah;
                
                $this->db->where('id', $cond['id_transaksi']);
                $update_trans = $this->db->update('transaksi', array('id_driver' => $cond['id_driver'], 'wilayah_driver'=> $wil));
    ...
    public function finish_request($cond, $condtr)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->promo_customer($last_trans->row('order_fitur'), $last_trans->row('id_pelanggan'));
    ...
    function cut_driver_saldo_by_order($data)
    {
        ...
        $data_poin = array(
          ...
          "trans_id" => $data['id_transaksi'],
          "driver_id" => $data['id_driver']
        );
        
        
        $this->db->insert('point_transaction', $data_poin);
        ...
        if ($data['pakai_wallet'] == 1) {
            ...
            if ($ins_trans) {
                ...
                if ($this->db->affected_rows() > 0) {
                    ...
                    if($data['order_fitur'] =='3' || $data['order_fitur'] =='9')
                    {
                        $this->feefood($data);
    ...
    public function promo_customer($id, $idcust)
    {
        $this->db->where('id_fitur', $id );
        $fitur = $this->db->get('fitur')->row();
        $harga_promosi = $fitur->promo_customer;
        
        $this->db->query("UPDATE saldo SET saldo = saldo + '$harga_promosi' WHERE id_user ='$idcust'");
        $array = array(
          "id_user" => $idcust,
          "jumlah" => $harga_promosi,
          "bank" => 'Cash Back',
          "nama_pemilik" => 'insentif customer',
          "rekening" => "wallet",
    ...
    function insentif($data)
    {
       $iduser = $data['id_driver'];
       $tanggal = date('Y-m-d H:i:s');
       $jumlah = 1000; 
       ...
       $data_insert = array(
        ...
        $response = $this->db->insert('wallet', $data_insert);
        if($response)
        {
             $this->db->query("UPDATE saldo SET saldo = saldo + $jumlah, update_at = '$tanggal' WHERE id_user = '$iduser' ");
        }
    }
    ...
    function feefood($data)
    {
        $id_transaksi = $data['id_transaksi'];
        $this->db->where('id_transaksi', $id_transaksi);
        $query = $this->db->get('transaksi_detail_merchant')->result();
        $pesanan = $query[0]->total_biaya;
        $bayar = $pesanan / 1.1 ;
        $fee = $pesanan - $bayar;
        $bagianadmin = $fee/2;
        ...
        if($response)
        {
            if($data['pakai_wallet'] == 1)
            {
                $this->db->query("UPDATE saldo SET saldo = saldo + $bagianadmin, update_at = '$tanggal' WHERE id_user = '$id_user' ");
            }
            else
            {
                $this->db->query("UPDATE saldo SET saldo = saldo - $bagianadmin, update_at = '$tanggal' WHERE id_user = '$id_user' ");
    ...
    public function getAlldriverPages($number,$offset)
    {
        $this->db->select('config_driver.status as status_job');
        $this->db->select('driver_job.driver_job');
        $this->db->select('driver.*');
    ...
    public function ubahdatacard($data, $data2, $data3)
    {
        ...
        $this->db->set('foto_stnk', $data3['foto_stnk']);
        $this->db->set('foto_kendaraan', $data3['foto_kendaraan']);
        $this->db->where('id_driver', $data3['id']);
        $this->db->update('berkas_driver');
    ...
    public function driverlocation($id_driver)
    {
        $this->db->where('id_driver', $id_driver);
    ...
    public function idpelanggan($idtransaksi)
    {
    ...
    // DATATABLE
	var $table ="driver d";
	var $join_config = "config_driver cd";
	var $join_job = "driver_job dj";
    
    var $select_column = array(
    	"cd.status as status_job", 
    	"dj.driver_job",
    	"d.*",
    ...
    function make_query($type)
    {
        ...
        $this->db->join($this->join_config, "d.id = cd.id_driver", "left");
        $this->db->join($this->join_job, "d.job = dj.id", "left");
        ...
        if($type == 'driver_aktif')
        {
            $this->db->where('d.status !=', 3);
            $this->db->group_start();
            $this->db->where('cd.status', 1);
            $this->db->or_where('cd.status', 2);
            $this->db->or_where('cd.status', 3);
            $this->db->group_end();
        }
        
        if($type == 'driver_nonaktif')
        {
            $this->db->group_start();
            $this->db->where('cd.status', 4);
            $this->db->or_where('cd.status', 5);
            $this->db->group_end();
        ...
        if(isset($_POST["order"]))
        {
            if($type == 'all_driver')
            {
                $this->db->order_by($this->order_fields[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
            }
            else
            {
    ...
    function make_datatables($type)
    {
        $this->make_query($type);
        if($_POST["length"] != -1){
