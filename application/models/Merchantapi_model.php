...
class Merchantapi_model extends CI_model
{
    ...
    public function fitur_merchant_active($idwilayah)
    {
        
        $this->db->select('id_fitur,fitur, wilayah');
        
        $this->db->where('home = 4');
        $this->db->where('active', '1');
        $this->db->order_by('id_fitur ASC');
        $query = $this->db->get('fitur')->result_array();
        
        $rows = array();
        foreach($query as $key)
        {
            $wilayahstring = $key['wilayah'];
            $wilayaharr = explode(",", $wilayahstring);
            
            for($i=0; $i<count($wilayaharr); $i++)
            {
                if($wilayaharr[$i] == $idwilayah) 
                {
